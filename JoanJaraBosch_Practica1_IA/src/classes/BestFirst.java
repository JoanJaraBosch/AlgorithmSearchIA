...
public class BestFirst {
     ...
     * @param gps
     */
    public BestFirst(int x, int y, int[][] gps, int heuristica) {
        this.x=x;
        this.y=y;
        this.gps=gps;
    ...
    public void south(LinkedList<Section> toTreath, Section currently, LinkedList<Section> treatened,Section finalSection) {
        ...
        if((vX+1)<x) {
            if(gps[vX+1][vY]!=-1) {
                ...
                if(!isNodeTreated(newSection4,treatened) && !isNodetoThreat(newSection4,toTreath)) {
            	    LinkedList<int[]> roadNew = (LinkedList<int[]>) currently.getRoad().clone();
                    roadNew.add(aux);
                    Section newSection2 = new Section(gps[vX+1][vY], vX+1, vY,roadNew);
                    float valueToOrder=heuristica(heuristica,newSection2,finalSection);
                    Section newSection3 = new Section(gps[vX+1][vY], vX+1, vY,roadNew,valueToOrder);
                    toTreath.add(newSection3);
                    orderQueue(toTreath);
                }
            }
        }
        ...
        if((vY+1)<y){
            if(gps[vX][vY+1]!=-1) {
                ...
                if(!isNodeTreated(newSection4,treatened) && !isNodetoThreat(newSection4,toTreath)) {
                    ...
                    float valueToOrder=heuristica(heuristica,newSection2,finalSection);
                    Section newSection3 = new Section(gps[vX][vY+1], vX, vY+1,roadNew,valueToOrder);
                    toTreath.add(newSection3);
                    orderQueue(toTreath);
                }
            }
    ...
    /**
     * Metode que serveix per a mirar si un estat, esta a la llista de pendents o no
     * @param node
     * @param treated
     * @return retorna true o fals depenent de si ha estat tractat o no
     */
    public boolean isNodetoThreat(Section node, Queue<Section> treated) {
        for (Section actual : treated) {
            if (node.getPosition_x() == actual.getPosition_x() &&
                    node.getPosition_y() == actual.getPosition_y())
                return true;
    ...
    public float heuristica(int heuristica, Section current, Section finalSection) {
    	float valor=0;
    	if(heuristica==1) {
